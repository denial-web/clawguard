...
ClawShield can run in pull requests as a policy gate and emit SARIF for GitHub code scanning.
...
- `config`: optional `.clawshield.json` path
- `sarif`: optional SARIF output path. Default: `clawshield.sarif`
...
- Use `if: always()` for SARIF upload so GitHub receives results even when ClawShield fails the policy gate.
