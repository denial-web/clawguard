...
|---|---|---|---|
| NeuZhou/clawguard ([npm](https://www.npmjs.com/package/@neuzhou/clawguard); source repo not publicly accessible as of 2026-05-26 — see [COMPARISON.md](COMPARISON.md)) | CLI + library + HTTP server; SARIF; LangChain middleware. | Nothing direct; we both emit SARIF, so both can target the same GitHub Code Scanning surface. | **Parallel.** Both can run on the same repo without conflict. No shared config schema. |
